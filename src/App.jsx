...
function App() {
  ...
  useEffect(() => {
    const loadPokemonNames = async () => {
      try {
        const pokemonNames = await getPokemons()
  ...
  return (
    <div className="bg-background h-screen">
      <div className="container lg:w-[720px] mx-auto p-4">
        <Search onSearch={handleSearch} suggestions={suggestions} />
        {loading && (
          <div class="text-center">
            <div role="status">
              <svg aria-hidden="true" class="inline w-8 h-8 text-gray-200 animate-spin fill-primary" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {!loading && data && <PokemonCard pokemon={data} />}
      </div>
      <Footer />
    </div>
  )
